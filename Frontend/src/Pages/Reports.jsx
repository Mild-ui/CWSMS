...
export default function Reports() {
    ...
    const fetchReports = () => {
        axios.get('http://localhost:5000/retrieve-report')
            .then(res => {
        ...
        <div className='flex h-screen bg-slate-50 font-sans'>
            ...
            <main className='flex-1 p-10 overflow-y-auto space-y-6'>
                <div className='heading'>
                    ...
                    <p className='text-slate-500 ml-1'>Comprehensive management audit reports and operational analytics logs.</p>
                </div>

                <div className="packages space-y-4">
                    <h1 className='text-2xl font-extrabold text-slate-800 tracking-tight'>System<span className='text-amber-500 font-medium italic font-serif'>Reports</span></h1>
                        ...
                        <table className='w-full text-left border-collapse'>
                            <thead>
                                <tr className="bg-slate-50 border-b border-slate-100 text-xs font-semibold uppercase tracking-wider text-slate-400">
                                    ...
                                    <th className='px-3 py-6'>Car Type</th>
                                    <th className='px-3 py-6'>Service Package</th>
                                    <th className='px-3 py-6'>Amount Paid</th>
                                    <th className='px-3 py-6'>Service Date</th>
                                </tr>
                            </thead>
                            <tbody className='divide-y divide-slate-100 text-sm text-slate-700'>
                                {reportData.map((report, index) => (
                                    <tr key={report.PaymentNumber || index} className="hover:bg-slate-50/70 transition-colors">
                                        <td className='py-4 px-6 font-mono text-xs text-slate-400'># {report.PaymentNumber}</td>
                                        <td className='py-4 px-6 font-medium text-slate-900'>{report.DriverName}</td>
                                        <td className='py-4 px-6'>{report.PlateNumber}</td>
                                        <td className='py-4 px-6'>{report.CarType} ({report.CarSize})</td>
                                        <td className='py-4 px-6'>{report.PackageName}</td>
                                        <td className="py-4 px-6 font-semibold text-slate-900">{report.AmountPaid} Rwf</td>
                                        <td className='py-4 px-6'>{report.ServiceDate}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
